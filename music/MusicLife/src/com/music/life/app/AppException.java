	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			public void run() {
				...
				parameters.add(new BasicNameValuePair("crashReport", crashReport));
				try {
					// Thread.sleep(2 * 1000);
					AppClient.getAppClient().http_post(Urls.APP_CRASH_REPORT_URL, parameters);
				} catch (Exception e) {
					e.printStackTrace();
				}
				new SweetAlertDialog(context, SweetAlertDialog.ERROR_TYPE).setTitleText("程序已出错，请关闭重试").setConfirmText("　退出程序　")

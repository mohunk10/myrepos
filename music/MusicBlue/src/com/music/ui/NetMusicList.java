...
import com.music.uifragment.NetMusicFrag;
...
public class NetMusicList extends BaseActivity {
	...
	private void init() {
		...
		String musickey = intent.getStringExtra(MUSIC_KEY);
		String paramtype = intent.getStringExtra(PARAM_TYPE);
		mPublicParams.put(MUSIC_KEY, TextUtils.isEmpty(musickey) ? "" : musickey);
		mPublicParams.put(PARAM_TYPE, TextUtils.isEmpty(paramtype) ? "" : paramtype);
	}
					...
					mBinder.setOnPlayerListener(new OnPlayerListener() {
						...
						public void onPlay(int currentPosition) {
							if (null == mCurrentMusicInfo)
								return;
							// String mp3Current =
							// FormatUtil.formatTime(currentPosition);
	...
	private void initView() {
		mFragment = new NetMusicFrag();
		getSupportFragmentManager().beginTransaction()
				.add(R.id.activity_fragment, (Fragment) mFragment, "activity_fragment").commit();
	}

	@Override
	protected void initTitleBar() {
		super.initTitleBar();
		tv_titlebar_title.setText("歌曲列表");
	}

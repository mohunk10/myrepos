...
//			encode = set.name();
		...
		InputStreamReader inputReader = new InputStreamReader(inputStream, encode);
		...
		while ((temp = bufferedReader.readLine()) != null) {
			...
			if (count > 0) {
				...
				for (int j = 0; j < timeTemp.size(); j++) {
					...
					else if (timeTemp.get(j) > lyricList.get(lyricList.size() - 1).getTime()) {
						item.setTime(timeTemp.get(j));
						if (line == 1) {
							msg = temp.substring(timeLength * count + 1);
						} else {
					...
					// 否则按大小顺序插入
						...
						for (int index = 0; index < lyricList.size(); index++) {
							if (timeTemp.get(j) <= lyricList.get(index).getTime()) {
								item.setTime(timeTemp.get(j));
								if (line == 1) {
									msg = temp.substring(timeLength * count + 1);
								} else {

...
public class LyricView extends TextView {
	...
	private float textSize = 45;// 字体大小
	...
	private int[] paintColorsCurrent = { Color.argb(250, 251, 248, 29), Color.argb(250, 255, 255, 255) };// 卡拉OK模式绘画中画笔颜色数组
	private int[] paintColorsDefault = { Color.argb(250, 255, 255, 255), Color.argb(250, 255, 255, 255) };// 卡拉OK模式默认画笔颜色数组

	private boolean isKLOK = false;

	private int mHalfLineSpacing = 10;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (canvas == null || lyricSize <= 0 || index >= mSentenceEntities.size()) {
			return;
		}
		...
		if (isKLOK) {
			...
			float position = dunringTime == 0 ? 0 : ((float) currentTime - (float) startTime) / (float) dunringTime;// 计算当前位置

			if (index % 2 == 0) {
				float start1 = len / 2;// 第一句的起点位置
				LinearGradient gradient = new LinearGradient(0, 0, len, 0, paintColorsCurrent, new float[] { position, position }, TileMode.CLAMP);// 重绘渐变
				currentPaint.setShader(gradient);
				canvas.drawText(text, start1, tempYHigh - mHalfLineSpacing, currentPaint);
				...
				if (nextIndex < lyricSize) {
					...
					gradient = new LinearGradient(start2, 0, width, 0, paintColorsDefault, null, TileMode.CLAMP);// 重绘渐变
					defaultPaint.setShader(gradient);
					canvas.drawText(text, start2, tempYLow + mHalfLineSpacing, defaultPaint);
				}
			} else {
				float start2 = width - len / 2;// 第二句的起点位置
				float w = width > len ? width - len : 0;// 第二句的渐变起点位置
				LinearGradient gradient = new LinearGradient(w, 0, width, 0, paintColorsCurrent, new float[] { position, position }, TileMode.CLAMP);// 重绘渐变
				defaultPaint.setShader(gradient);
				canvas.drawText(text, start2, tempYLow + mHalfLineSpacing, defaultPaint);
				...
				if (nextIndex < lyricSize) {
					...
					len = this.getTextWidth(currentPaint, text);// 该句歌词精确长度
					float start1 = len / 2;// 第一句的起点位置
					gradient = new LinearGradient(0, 0, len, 0, paintColorsDefault, null, TileMode.CLAMP);// 重绘渐变
					currentPaint.setShader(gradient);
					canvas.drawText(text, start1, tempYHigh - mHalfLineSpacing, currentPaint);
				}
			}
		} else {
			float plus = dunringTime == 0 ? 0 : (((float) currentTime - (float) startTime) / (float) dunringTime) * (float) 30;
			// 向上滚动 这个是根据歌词的时间长短来滚动，整体上移
			canvas.translate(0, -plus);

			try {
				canvas.drawText(mSentenceEntities.get(index).getLyric(), tempW, tempH, currentPaint);
				...
				for (int i = index - 1; i >= 0; i--) {
					...
					canvas.drawText(mSentenceEntities.get(i).getLyric(), tempW, tempY, defaultPaint);
				}
				tempY = tempH;
				// 画出本句之后的句子
	 ...
	 */
	public void clear() {
		this.mSentenceEntities = new ArrayList<LyricItem>();
		this.index = 0;
		this.lyricSize = 0;
		this.invalidate();
	}
